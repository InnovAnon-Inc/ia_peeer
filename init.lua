...
local pee_color  = '#DDD618'
local node_alpha = 100 -- use default
local alpha      = 150 -- use default
...
function ia_peeer.set_urine_amount(playername, amount)
    assert(playername ~= nil)
    assert(amount     ~= nil)
    minetest.log('ia_peeer.set_urine_amount(playername='..playername..', amount='..amount..')')
end

function ia_peeer.decrement_urine_amount(playername, amount)
    assert(playername ~= nil)
    assert(amount     ~= nil)
    minetest.log('ia_peeer.decrement_urine_amount(playername='..playername..', amount='..amount..')')
end
...
function ia_peeer.spawn_pee_droplet_entity(spawn_pos, look_dir, freq)
    local obj         = minetest.add_entity(spawn_pos, modname..":urine_droplet")
    if not obj then return nil end
...
local function on_step(self, dtime)
    ...
    local ray = minetest.raycast(pos, fin, false, false)
...
local function on_impact_primary(pos)
    if not minetest.get_modpath('pedology') then return false end
    pos            = vector.round(pos)
    local expected = minetest.get_node(pos)
    pedology.wetten(pos)
...
local function on_impact_secondary(pos)
	...
	minetest.log('cannot pee at '..tostring(pos))
        pos        = vector.add(pos, above)
    end
    if not can_pee_on(pos) then
	minetest.log('cannot pee on '..tostring(pos))
         return false
    end
    local expected = minetest.get_node(pos)
    ...
    minetest.set_node(pos, {name = modname..":urine_flowing", param2 = 1})
    local actual   = minetest.get_node(pos)
    return (actual ~= expected)
end
...
local function on_impact(self, pos)
    ...
    if wetted then return end
    minetest.log('ia_peeer.on_impact failure')
end
...
minetest.register_chatcommand("urinate", {
    params = "[amount]",
...
minetest.register_entity(modname..":urine_droplet", {
    initial_properties = {
        visual = "sprite",
        textures = {droplet},
...
--})
--
--minetest.register_node("ia_peeer:urine_flowing", {
